...
const VoucherCard = () => {
  ...
  console.log(data.data.voucher_id);

  const handlePrint = () => {
    printJS({
      printable: "printArea",
      type: "html",
      documentTitle: `Voucher ID-${data.data.voucher_id}`,
      css: [
  ...
  const handlePDF = async () => {
    ...
    const options = {
      margin: 0,
      filename: `Invoice-${data.data.voucher_id}.pdf`,
      ...
      <div ref={componentRef} id="printArea" className="w-[14.8cm] p-8">
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <h3 className="text-xl">{data.data.voucher_id}</h3>
          </div>
          <div className="text-right">
            <h3 className="text-xl font-bold">Bill to</h3>
            <p className="text-xl">{data.data.customer_name}</p>
