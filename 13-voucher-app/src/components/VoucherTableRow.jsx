...
const VoucherTableRow = ({
  ...
  const handleDecreaseQtyBtn = (recordId) =>{
    ...
    // console.log(currentRecord.quantity)
    if(currentRecord.quantity === 0){
      deleteRecord(recordId)
      toast.success(`${product_name} has been removed!`)
    }
  }
  ...
  return (
    <tr className="group odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
      ...
      <td className="px-6 py-4 text-end">
        <button onClick={() => handleDecreaseQtyBtn(id)} className="q-sub pointer-events-none group-hover:pointer-events-auto opacity-0 group-hover:opacity-100 -translate-x-6 group-hover:translate-x-0 duration-200 bg-blue-100 text-blue-600 p-1 rounded">
          <svg
        ...
        <span className="record-q inline-block w-7 text-center">{quantity}</span>
        <button onClick={() => handleIncreaseQtyBtn(id)} className="q-add pointer-events-none group-hover:pointer-events-auto opacity-0 group-hover:opacity-100 translate-x-6 group-hover:translate-x-0 duration-200 bg-blue-100 text-blue-600 p-1 rounded">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
